...
以上列出来的都是集群核心组件，我们使用二进制安装包进行安装。Linux二进制按照包的安装过程通常是把二进制安装包解压到一个指定的目录，再准备好配置文件或设置好环境变量，最后再启动服务。这种安装方式让我们更熟悉整个服务的启动流程，因此也更加可控。而非集群的核心程序我们使用apt工具安装即可，如后续会用到的`vim`、`ipvsadm`、`bind9`等。
